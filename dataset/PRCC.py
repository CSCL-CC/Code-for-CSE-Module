...
class PRCC(BaseImageDataset):
    ...
    def __init__(self, root='', cse_root='', verbose=True, **kwargs):
        ...
        train = self._process_dir(self.train_dir, relabel=True,training=True,cam=None)
        query = self._process_dir(self.query_dir, relabel=False,cam = 'B')
        gallery = self._process_dir(self.gallery_dir, relabel=False,cam = 'A')
        ...
        self.change_query = self._process_dir(self.query_dir, relabel=False,cam = 'C')
    ...
    def _process_dir(self, dir_path, relabel=False,training = False,cam = None):
        ...
        for img_path in img_paths:
            ...
            if not img_path.split('/')[-1].split('_')[1] ==cam and not training:
                continue
            pid = int(img_path.split('/')[-1].split('_')[0])
            if img_path.split('/')[-1].split('_')[1] == 'A':
                clothid = 0
                camid = 0
            elif img_path.split('/')[-1].split('_')[1] == 'B':
                if training:
                    clothid = 0
                    camid = 0
                else:
                    clothid = 2
                    camid = 2
            elif img_path.split('/')[-1].split('_')[1] == 'C':
                clothid = 1
                camid = 1
