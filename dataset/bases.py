...
class BaseDataset(object):
    ...
    def get_imagedata_info(self, data):
        pids, cams, clothids = [], [], []
        for img_path, mask_path, pid, clothid, all_clothid,camid in data:
            pids += [pid]
            cams += [camid]
            clothids += [all_clothid]
        ...
        num_pids = len(pids)
        num_cams = len(cams)
        num_cloths = len(clothids)
        num_imgs = len(data)

        return num_pids, num_imgs, num_cams,num_cloths
...
class BaseImageDataset(BaseDataset):
    ...
    def print_dataset_statistics(self, train, query, gallery):
        num_train_pids, num_train_imgs, num_train_cams, train_cloths = self.get_imagedata_info(train)
        num_query_pids, num_query_imgs, num_query_cams, query_cloths = self.get_imagedata_info(query)
        num_gallery_pids, num_gallery_imgs, num_gallery_cams, gallery_cloths = self.get_imagedata_info(gallery)

        print("Dataset statistics:")
        print("  ----------------------------------------")
        print("  subset   | # ids | # images | # cameras | # cloths|")
        print("  ----------------------------------------")
        print("  train    | {:5d} | {:8d} | {:9d}| {:9d}|".format(num_train_pids, num_train_imgs, num_train_cams, train_cloths))
        print("  query    | {:5d} | {:8d} | {:9d}| {:9d}|".format(num_query_pids, num_query_imgs, num_query_cams, query_cloths))
        print("  gallery  | {:5d} | {:8d} | {:9d}| {:9d}|".format(num_gallery_pids, num_gallery_imgs, num_gallery_cams, gallery_cloths))
        print("  ----------------------------------------")
