import chumpy
import trans_utils
from smpl_utils import posemap
...
def verts_decorated(trans, pose, 
    ...
    assert(bs_style=='lbs')
    result, Jtr = trans_utils.verts_core(pose, v, J, weights, kintree_table, want_Jtr=True, xp=chumpy)
...
def verts_core(pose, v, J, weights, kintree_table, bs_style, want_Jtr=False, xp=chumpy):
    ...
    assert(bs_style=='lbs')
    result = trans_utils.verts_core(pose, v, J, weights, kintree_table, want_Jtr, xp)

    return result

if __name__ == '__main__':
    import numpy as np
    x = np.arange(-3.0, 3.0, 0.125)
    print(x)
